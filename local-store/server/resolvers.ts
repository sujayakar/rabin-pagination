...
import { ExactKey, IndexKey, isMaximal, isMinimal, Key, MAXIMAL_KEY, MINIMAL_KEY, PageResult } from "../shared/types";
import { AnyDataModel, GenericDocument, GenericQueryCtx, queryGeneric } from "convex/server";

export const ltOr = (equal: boolean) => (equal ? "lte" : "lt");
export const gtOr = (equal: boolean) => (equal ? "gte" : "gt");

export function rangeToQuery(range: Bound[]) {
  return (q: any) => {
...
export function paginationResolver(indexFields: string[], indexResolver: IndexResolverGenerator) {
  return queryGeneric({
    args: {
      target: v.any(),
    ...
    handler: async (ctx, args) => {
      ...
      const getCursor = (doc: GenericDocument) => {
        return {
          ...
          kind: "exact" as const,
          value: indexFields.map((field: string) => doc[field] ?? null),
        };
      };
